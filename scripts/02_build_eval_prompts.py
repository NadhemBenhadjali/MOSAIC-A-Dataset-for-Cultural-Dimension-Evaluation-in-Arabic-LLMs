#!/usr/bin/env python
"""Create standard two-option evaluation prompts."""
...
if __name__ == "__main__":
    args = parse_args()
    output_dir = RESULTS_DIR / "prompts" / "standard_eval"

    for code, dimension in DIMENSIONS.items():
        df = read_csv(DATA_DIR / "processed" / dimension.source_file)
        prompts = create_eval_prompts(df, dimension=code, seed=args.seed)
        output_file = output_dir / f"{code}_standard_prompts.csv"
